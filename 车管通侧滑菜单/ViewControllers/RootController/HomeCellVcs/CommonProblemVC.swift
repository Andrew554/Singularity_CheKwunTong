//
//  CommonProblemVC.swift
//  车管通侧滑菜单
//
//  Created by SinObjectC on 15/12/9.
...
class CommonProblemVC: BaseViewController, UISearchBarDelegate, UITableViewDelegate, UITableViewDataSource {
    var tv: UITableView!
    ...
    func initUI() {
        ...
        self.tv.registerClass(UITableViewCell.self, forCellReuseIdentifier: "searchCell")
    ...
    func requestData() {
        ...
        Alamofire.request(.GET, "\(rootPath)\(allProblem)").validate().responseJSON {
            (response)-> Void in
            switch response.result {
            case .Success(let value):
                let json = JSON(value).arrayValue
                for arr in json {
                    let dic = NSDictionary(objects: ["\(arr["id"])", "\(arr["problem_title"])"], forKeys: ["id", "problem_title"])
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell?.textLabel?.text = self.lists![indexPath.row]["problem_title"] as? String
